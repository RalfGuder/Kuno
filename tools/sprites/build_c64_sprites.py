...
def build_inc(cfg: Config) -> str:
    """Generate TRSE-Pascal @define lines: two per configured phase.

    Each phase produces an _OUTLINE define pointing at the lower-bank
    hardware ID (sprite_index_base + slot) and a _FILL define pointing
    at the upper-bank hardware ID (sprite_index_base + half + slot),
    where half = total_slots // 2.
    """
    lines = [
